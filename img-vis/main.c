...
void print_image(SDL_Surface *surface) {
    ...
    SDL_Surface *rgb_surface = SDL_ConvertSurfaceFormat(surface, SDL_PIXELFORMAT_RGBX8888, 0);
    // SDL_FreeSurface(surface);
    float scale = 1;
    int new_height, new_width;
    float ratio = (float)img_width / (float)img_height;
    char *density_map = "$@B%8&WM#*oahkbdpqwmZO0QLCJUYXzcvunxrjft/\\|()1{}[]?-_+~<>i!lI;:,^`'. ";
    ...
    SDL_Surface *resized = SDL_CreateRGBSurfaceWithFormat(
        0, new_width, new_height, rgb_surface->format->BitsPerPixel, rgb_surface->format->format);
