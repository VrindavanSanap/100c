...
void read_and_print_uint32(
    FILE* file, uint32_t* value, const char* error_message, const char* label)
{
...
void switch_set()
{
    ...
    read_and_print_uint32(image_file, &magic_number, "failed to read magic number", "Magic number");
    read_and_print_uint32(image_file, &num_items, "failed to read num_items", "Num items");
    read_and_print_uint32(image_file, &n_rows, "failed to read n rows", "Num rows");
    read_and_print_uint32(image_file, &n_cols, "failed to read num_cols", "Num cols");
    train_set = !train_set;
}
...
int main()
{
    ...
    if (!train_image_file || !train_label_file || !test_image_file || !test_label_file)
    {
